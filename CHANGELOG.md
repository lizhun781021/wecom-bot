...
**QQ 权限确认通知可靠性增强 + 8505 面板待确认看板 + 长任务超时优化**。

### 背景
准哥在 QQ 私聊触发抖音视频下载（需安装下载工具，属敏感操作），后台生成了权限确认请求，但 QQ 通道当时断线（botpy Session timed out）+ 8088 SSE 超时，导致"请回复确认/拒绝"通知**发了但没送达**，准哥在 QQ 里没看到。且无留档，重启后丢失。

### 修复
- **权限确认通知同步发送 + 失败标记**（qq_official_adapter.py）：
  - 新增 `_reply_text_sync()`，同步等待发送结果（15s 超时），发送失败时标记 `notice_sent=False`
  - 通知发送失败不再"发了就算成功"，而是记录到待确认池等待补发
- **待确认请求持久化**：
  - 新增 `qq_pending_confirmations.json`，待确认请求写入文件，重启后仍可匹配用户回复
  - 注册 / 确认 / 清理 / 接收回复时同步持久化
- **断线恢复自动补发**：
  - 新增后台线程 `_pending_notice_retry_loop()`，每 30s 扫描未送达的待确认请求
  - 连接恢复后通过 API 主动补发，最多重试 5 次
  - 适配器启动时立即启动该线程
- **8505 面板新增"待确认权限请求"看板**（dashboard.py）：
  - 新增 API `/api/qqpending`，返回当前待确认列表（含类型/场景/会话/通知内容/送达状态/等待时长）
  - QQ 通道卡片下方动态展示待确认卡片，有空闲时自动隐藏
  - 送达状态可视化：已送达 / 待补发（重试次数）
- **长任务超时策略优化**：
  - `call_teleagent` timeout 从 1800s（30分钟）降至 600s（10分钟），减少长任务挂起时间
  - 配合 v1.15.2 的忙碌保护，超时后立即释放会话计数，用户可重新发起请求

---

## v1.15.2 (2026-08-28)

**修复 QQ 私聊"不流式"真正根因：同一会话长任务执行期间新消息排队挂起**。

### 背景
v1.15.1 修复了进度消息占用被动通道的问题，但准哥反馈 QQ 私聊仍会"没说几句就不流了"。深入排查发现第二层根因：
- QQ 私聊/群聊按 `session_title` 固定复用同一 8088 会话（`QQ|私聊|昵称|时间`）
- 用户发起长任务（如"评价网页 PPT"→ 浏览器截图 57+ 次工具调用）时，该会话被**持续占用 30+ 分钟**
- 期间用户再发新消息 → 8088 按标题复用同一会话 → **新请求排队等待长任务完成** → 8088 SSE 超时（600s）→ 表现为"没有流式输出"

### 修复
- **QQ 适配器会话忙碌保护**（qq_official_adapter.py）：新增 `_qq_session_active` 活动请求计数，按用户/群维度（`c2c:{openid}` / `group:{group_openid}`）隔离，同一会话同时只允许 1 个请求
- 忙碌时**立即回复提示**"⏳ 您上一条消息还在处理中…请稍等片刻"，而不是挂起排队（新请求不再占 8088 会话）
- **权限确认优先放行**：用户回复"确认/拒绝"不受忙碌保护限制（否则长任务等待确认时会永久卡死）
- 请求结束（成功/失败/超时）在 `finally` 中释放会话计数，避免永久占用

### 说明
- 企微通道（server.py）存在同类机制问题（同会话并发排队），后续版本将按相同模式扩展

---

## v1.15.1 (2026-08-28)

**修复 QQ 私聊流式输出中断：进度消息占用被动回复通道导致后续消息被平台去重**。

### 修复
- **QQ 私聊流式中断（40054005）**：私聊被动回复通道同一 `msg_id` 只能发一条消息（`msg_seq` 固定 1），此前 `_qq_on_delta` 每 5 秒发"正在生成中…"进度消息会占用被动通道，导致后续进度和最终完整回复全部被 QQ 平台以「消息被去重」拒绝，表现为"流到一半不流了"。现改为**私聊禁用进度推送**，最终回复作为唯一一条被动消息发出，群聊保持原进度推送逻辑（`msg_seq` 递增不受限）
